...
An example of a eslint config is as below.

```javascript
module.exports = {
  parser: 'specify parser type here',
  parserOptions: {
    option1: 'some option setting',
    option2: 'some option setting 2',
  },
  env: {
    'some env': true,
    'another': true,
...
The `lint-staged` setting will run the `lint:fix` script on any staged `*.js` or `*.jsx` file. Once all the files have been fixed, the fixes will be added to staging via `git add`. Next, all staged files will be committed.

Now try making some changes to a `.js` file, stage it and commit, You will see lint-stage being executed.
